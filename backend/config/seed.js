...
async function seed() {
  ...
  for (const authorData of sampleData.authors) {
    const user = new User({
      ...
      joined_date: authorData.joined_date,
      role: 'author'
    });
    await user.save();

    for (const bookData of authorData.books) {
      const book = new Book({ ...bookData, author: user._id });
      await book.save();
    }
    console.log(`Created author: ${authorData.email} / author123`);
  }

  console.log('\nSeeding complete!');
  console.log('Admin login: admin@bookleaf.com / admin123');
  console.log('Author login example: priya.sharma@email.com / author123');
  process.exit(0);
}
